...
class Example:
    sentence: str
    actual_labels: (
        dict  # {"action": "jump", "target": "left", "correction_connector": None}
    )
...
def get_dataset_paths(dataset):
    ...
    root = DATA_ROOT / "open" / "vocab_test"
    return (
        root / "simple" / "test" / "*.json",
        root / "complex" / "test" / "*.json",
    )
...
def decode_bio_spans(
    sentence: str, tokenizer, id2label, logits, offset_mapping, word_ids
) -> dict:
...
def run_bert_on_example(model, tokenizer, sentence: str) -> tuple[dict, float]:
    ...
    raw_spans = decode_bio_spans(
        sentence, tokenizer, id2label, logits, offset_mapping, word_ids
    )
...
def benchmark_latency(
    examples: list[Example], model, tokenizer, n_passes: int = 5
) -> dict:
    all_latencies = []
...
def main():
    ...
    parser.add_argument("--dataset", choices=["closed", "open"], required=True)
    parser.add_argument("--output", type=Path)
    args = parser.parse_args()

    simple_path, complex_path = get_dataset_paths(args.dataset)

    simple_examples = load_dataset(str(simple_path), "simple")
    complex_examples = load_dataset(str(complex_path), "complex")
    ...
    output_path = args.output or REPO_ROOT / f"gamebert_{args.dataset}_results.json"
